...
retrieve_stock_market_indicators_job_schedule = ScheduleDefinition(
    job=make_job_from_graph(retrieve_stock_market_indicators_job), cron_schedule="0 6 * * 2-6", default_status=DefaultScheduleStatus.RUNNING
)

@repository
def get_etl_jobs():
    return [make_job_from_graph(hello_world_job), running_schedule,
            make_job_from_graph(retrieve_stock_market_indicators_job), retrieve_stock_market_indicators_job_schedule]

@repository
def get_ops_jobs():
    return [make_job_from_graph(deploy_job),
            make_job_from_graph(shell_job)]
